...
<header class="fixed top-0 left-0 right-0 z-50 backdrop-blur-lg bg-cream/80 border-b border-warm-100">
  <div class="max-w-2xl mx-auto flex items-center justify-between px-5 py-3">
    ...
    <div class="flex items-center gap-2">
      ...
      {:else if roomCode}
        <a href="/settings" class="flex items-center gap-1.5" title="{syncStatus}">
          <div class="w-1.5 h-1.5 rounded-full {statusColors[syncStatus]}"></div>
        </a>
      {/if}

      <a href="/settings" class="w-9 h-9 rounded-lg flex items-center justify-center text-ink-muted hover:bg-warm-100 transition-colors" aria-label={t('settings.title')}>
      ...
      <a href="/add" class="btn-primary !py-2 !px-4 !text-xs">
        {t('topbar.add')}
      </a>
    </div>
  </div>
